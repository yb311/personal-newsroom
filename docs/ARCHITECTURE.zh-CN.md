...
| | 内容 |
|---|---|
| **今日** | 专属今日摘要 + 顶部**「昨天到今天」板块**，按 Watch 分组列出「你关注的 X，新增了 Y」 |
| **快讯** | 实时短讯，只推过了你意图闸门的 |
...
**「进展」两种形态都做，但它们共用同一份数据。** 顶部板块是「今天新增了什么」的摘要视图，Watch 页的时间线是同一批里程碑的完整视图——底层都是 `WatchState.timeline` 加上「哪些是今天新增的」这个标记。所以不是做两遍，是同一份数据两种渲染。真正难的是判断「什么算新增」，那个只做一次。
...
interface WatchState {              // 「我已经告诉过你什么」—— 进展功能的基础
  ...
  //   每个 Milestone 带 firstSeenAt：顶部「昨天到今天」板块渲染 firstSeenAt 是今天的，
...
- 移植 `scripts/dev/doctor.ts` 的模式：跑管线前校验 key、回显所有开关和 tunable
- 移植 `NEWS_DISABLE_*` 系列 kill-switch，特别是 `NEWS_DISABLE_EVIDENCE` 的**复用本地快照**语义——它让离线调试几乎零成本，是老项目最实用的开发工具
- 移植 `dev/reliability/*.test.ts` 的 `node:test` 测试和三个体检工具（聚类 A/B 对比、聚类瓶颈体检、全文预算体检）
- **专门的召回体检**：给定一组人工标注「这条应该被 X 关注命中」的样本，量出 R1/R2/R3 各自的召回率和三路并集的召回率。这是验证「去掉关键词过滤是对的」的唯一硬证据，要在 M2 就建起来
- **无 key 路径单独验收**：全新安装、不填任何 key，能挑源、能抓、能读原文、能收藏。三个 AI tab 给的是清楚的说明而不是空白或报错
- **端到端验收**：勾一个「科技」标签 + 建一个「习近平最近在干什么」的意图 Watch（一个设中文输出、一个设英文）→ 两者都立刻出建仓简报 → 今日 tab 看到摘要和顶部「昨天到今天」→ 点进 Watch 看到完整时间线且今天新增的高亮 → 阅读 tab 能直接读到原文 → 对其中一条点「深入」拿到带时间线的内容总结，每句能点回原文 → 关掉软件、第二天早上打开，后台已经跑过了，能看到新的变化
