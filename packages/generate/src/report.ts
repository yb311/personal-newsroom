...
/** A turn still pending after a crash would block its conversation forever. */
export function recoverReports(db: Db): void {
  db.prepare("UPDATE conversation_messages SET status = 'failed', updated_at = ? WHERE status = 'pending'").run(Date.now());
}
...
export async function startReport(db: Db, provider: Provider, dataDir: string, input: ReportStartInput,
  emit: (event: ReportEvent) => void, signal?: AbortSignal): Promise<ReportConversation> {
  if (!input.restart) {
    // Only a report that was actually written is worth restoring; a failed or
    // cancelled first attempt is started again rather than shown forever.
    const restored = getReport(db, { anchorItemId: input.anchorItemId, lang: input.lang });
    if (restored?.messages.some((m) => m.role === 'assistant' && m.status === 'complete')) return restored;
  }
