...
| 缺口 | 影响 | 属于哪个里程碑 |
|---|---|---|
| **App 级单实例锁缺失** | `apps/desktop/src/main.ts` 没调 `app.requestSingleInstanceLock()`。现在只有 DB 操作锁（抓取/生成互斥），挡不住用户手滑开两个窗口——两个 UI 进程本身没事，但都能各自触发 worker 操作，虽然 DB 锁会拦下并发写，用户体验上是「点了没反应」而不是「第二个实例直接不弹出来」 | M6 |
