...
**M-1 到 M6 的功能已全部完成并验证。** 只剩打包签名公证（需 Apple 开发者账号）。
...
- RSSHub 的分发策略：`node_modules` 约 415MB，要么随包发要么首启下载（两条路 AGPL 下都合法）
- 自动更新、一键卸载、贡献指南
...
- **RSSHub 不能被 esbuild 打包**，必须 `--external:rsshub` 并以 node_modules
  形式随包分发。它用动态 glob（`import(\`./routes/${ns}/...\`)`）按路由分块，
  而且有顶层 await，打成 CJS 必然失败
